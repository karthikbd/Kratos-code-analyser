...
from backend.config import settings
from backend.rag.regulation_store import ensure_regulations_ready
from backend.agents.graph import analyze_file_agentic, analyze_fdic370_agentic
from backend.models.outputs import FileAnalysis, RepoAnalysis, FDIC370Analysis
from backend.agents.tools import build_lineage, detect_missing_controls_for_sources
...
async def _scan_one_file(
    ...
    async with semaphore:
        try:
            ...
            if len(code.encode()) > MAX_FILE_SIZE_BYTES:
                return None
            analysis = await analyze_file_agentic(path, code, language)
            return {"analysis": analysis, "source": code}
        except Exception:
            return None
...
async def analyze_github_repo(req: GithubRepoRequest):
    ...
    # results are dicts {analysis, source} or None
    items = [r for r in results if r is not None]
    if not items:
        raise HTTPException(500, "All files failed or were skipped.")

    per_file: List[FileAnalysis] = [it["analysis"] for it in items]
    # build filename -> source mapping for lineage and heuristics
    sources: Dict[str, str] = {it["analysis"].filename: it["source"] for it in items}

    total_loc = sum(f.lines_of_code for f in per_file)
    total_inline_controls = sum(f.inline_controls_count for f in per_file)

    regimes = {
        reg
        for f in per_file
        for reg in f.regulatory_focus
    }
    total_regulations_referenced = len(regimes)
    ...
    overall_summary = (
        f"Agentic analysis of {owner}/{repo} on branch {default_branch} "
        f"covered {len(per_file)} files ({total_loc} LOC) and detected "
        f"{total_inline_controls} inline controls across {total_regulations_referenced} regulatory lenses."
    )

    # Build cross-file lineage and missing-controls summary
    lineage = build_lineage(sources)
    missing_controls_summary = detect_missing_controls_for_sources(sources)
    ...
    repo_analysis = RepoAnalysis(
        ...
        files_analyzed=len(per_file),
        total_loc=total_loc,
        total_inline_controls=total_inline_controls,
        total_regulations_referenced=total_regulations_referenced,
        overall_summary=overall_summary,
        per_file=per_file,
        language_counts=language_counts,
        category_counts=category_counts,
        lineage=lineage,
        missing_controls_summary=missing_controls_summary,
    )
    return repo_analysis.model_dump()


# -----------------------------------------------------------------------
# FDIC 12 CFR Part 370 Compliance Analysis Endpoints
# -----------------------------------------------------------------------

class FDIC370TextRequest(BaseModel):
    filename: str
    source_code: str
    language: Optional[str] = "python"


class FDIC370GithubRequest(BaseModel):
    url: str           # single file /blob/ URL or repo URL
    scan_type: str = "file"   # "file" | "repo"


@app.post("/api/analyze/fdic370/files", response_model=FDIC370Analysis)
async def fdic370_analyze_files(files: List[UploadFile] = File(...)):
    """Analyze one or more uploaded files against FDIC 12 CFR Part 370 controls."""
    ...
    for upload in files:
        ...
        try:
            ...
            raise HTTPException(400, f"File {upload.filename} must be UTF-8 text")
        if not source.strip():
            continue
        fname = upload.filename or "uploaded_file.py"
        file_contents[fname] = source
        languages.append(detect_language(fname))

    if not file_contents:
        raise HTTPException(400, "No non-empty UTF-8 files provided")

    result = await analyze_fdic370_agentic(file_contents, languages)
    return result.model_dump()


@app.post("/api/analyze/fdic370/text", response_model=FDIC370Analysis)
async def fdic370_analyze_text(req: FDIC370TextRequest):
    """Analyze pasted source code against FDIC 12 CFR Part 370 controls."""
    if not req.source_code.strip():
        raise HTTPException(400, "source_code is empty")

    lang = req.language or detect_language(req.filename)
    result = await analyze_fdic370_agentic(
...
@app.post("/api/analyze/fdic370/github", response_model=FDIC370Analysis)
async def fdic370_analyze_github(req: FDIC370GithubRequest):
    """Fetch from GitHub and analyze against FDIC 12 CFR Part 370 controls."""
    url = req.url.strip()
    file_contents: Dict[str, str] = {}
    languages: List[str] = []

    if req.scan_type == "file" or "/blob/" in url:
        # Single file
        if "/blob/" not in url:
            raise HTTPException(
                400,
        ...
        fname = raw_url.split("/")[-1] or "code.py"
        try:
            async with httpx.AsyncClient(timeout=30, verify=False) as client:
                r = await client.get(raw_url, headers=github_headers())
                r.raise_for_status()
        except httpx.RequestError as exc:
            raise HTTPException(502, f"GitHub network error: {exc}")
        except httpx.HTTPStatusError as exc:
            raise HTTPException(exc.response.status_code, f"GitHub error: {exc}")
        code = r.text
        if not code.strip():
            raise HTTPException(400, "File is empty")
        ...
        # Full repo — reuse existing tree logic, limit to MAX_FILE_SIZE_BYTES each
        repo_url = url.rstrip("/")
        if repo_url.endswith(".git"):
            repo_url = repo_url[:-4]
        match = re.search(r"github\.com/([^/]+)/([^/\s]+)", repo_url)
        ...
        items = [i for i in tree if is_supported_file(i)][:30]
        semaphore = asyncio.Semaphore(MAX_CONCURRENT)
        async with httpx.AsyncClient(timeout=120, verify=False) as client:
            tasks = [
                _scan_one_file(client, owner, repo, default_branch, i, semaphore)
                for i in items
            ]
            results = await asyncio.gather(*tasks)
        ...
        for res in results:
            if res and res.get("source"):
                fname = res["analysis"].filename
                file_contents[fname] = res["source"]
                languages.append(res["analysis"].language)

        if not file_contents:
            raise HTTPException(404, "No scannable files found in repo")
